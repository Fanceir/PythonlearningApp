...
使用 _in_ 运算符检查列表项是否为列表的成员。请参阅下面的示例。
...
要将项添加到现有列表的末尾，我们使用 _append()_ 方法。
...
我们可以使用 _insert()_ 方法在列表中的指定索引处插入单个项。请注意，其他项将向右移动。_insert()_ 方法接受两个参数：索引和要插入的项。
...
使用 _pop()_ 方法删除指定索引（如果未指定索引，则删除最后一项）：
...
使用 _del_ 关键字删除指定索引，也可以用于删除索引范围内的项。它还可以完全删除列表
...
可以通过将其重新分配给新变量来复制列表: list2 = list1。现在，list2 是 list1 的引用，我们对 list2 进行的任何更改也将修改原始的 list1。但是有很多时候我们不想修改原始的列表，而是想要一个不同的副本。为了避免这个问题，我们使用 _copy()_。
...
- 使用 _extend()_ 方法
  _extend()_ 方法可以将列表附加到列表中。请参阅下面的示例。
...
要对列表进行排序，我们可以使用 _sort()_ 方法或内置函数 _sorted()_。_sort()_ 方法将列表项按升序重新排序并修改原始列表。如果 _sort()_ 方法的 reverse 参数为 true，则会按降序排列列表。
